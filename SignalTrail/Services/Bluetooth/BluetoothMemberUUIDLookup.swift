...
/// Generated from Bluetooth SIG public assigned numbers:
...
enum BluetoothMemberUUIDLookup {
  ...
  static func name(for uuid: String) -> String? {
    guard let value = parse(uuid) else { return nil }
    return names[value]
  }

  static func displayName(for uuid: String) -> String {
    guard let value = parse(uuid) else { return uuid }
  ...
  private static func parse(_ raw: String) -> UInt16? {
    let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
    let canonical = trimmed.hasPrefix("0X") ? String(trimmed.dropFirst(2)) : trimmed
    guard canonical.count == 4, let value = UInt16(canonical, radix: 16) else { return nil }
    return value
  }
}
